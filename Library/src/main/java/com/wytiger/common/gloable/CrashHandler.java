...
/**
 * 线程未捕获异常控制器, 用来处理未捕获异常的。
 * 如果程序出现了未捕获异常,默认情况下则会出现强行关闭对话框, 实现该接口并注册为程序中的默认未捕获异常处理
 * 这样当未捕获异常发生时，就可以做些异常处理操作 例如：收集异常信息，发送错误报告 等。
 * UncaughtException处理类,当程序发生Uncaught异常的时候,由该类来接管程序,并记录发送错误报告.
 * 
 * @author xuxiaoming 2014年9月28日
 */
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private void saveToSDCard(Throwable ex) throws Exception {
       ...
       String crashTime = df.format(new Date());
    	...
    	File file = getSaveFile(CRASH_LOG,crashTime + FILE_NAME_SUFFIX);
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        try {
            ...
            final String fileName = "hs-" + time + ".log";
