package com.wytiger.common.widget.pathview;
...
public class PathView extends View {
    ...
    private void getFromAttributes(Context context, AttributeSet attrs) {
        final TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.PathView);
//        Log.e("PathView", "ta = " + ta);
        
        try {
            if (ta != null) {
                paint.setColor(ta.getColor(R.styleable.PathView_pathColor, 0xff00ff00));
                paint.setStrokeWidth(ta.getFloat(R.styleable.PathView_pathWidth, 8.0f));
                svgResourceId = ta.getResourceId(R.styleable.PathView_svg, 0);
            }
        } finally {
            if (ta != null) {
                ta.recycle();
     ...
     * 使用svg文件里的颜色,这个设置会使setPathColor()方法无效
     */
    ...
    /**
     * Set the path color.
     * 当使用了useNaturalColors()方法时设置了使用svg的颜色这个设置无效无效
     *
